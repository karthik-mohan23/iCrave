    ...
    </div>
{
  cart.map(item => {
    ...
    <div key={item.id} className="grid grid-cols-4 my-4 max-w-3xl">
      <h2 >{item.name}</h2>
      <p className="justify-self-center self-center">{item.quantity}</p>
      <p className="justify-self-center self-center">{item.totalPrice}</p>
      <button className="text-xs justify-self-end self-center bg-yellow-300 px-4 py-2 rounded-full tracking-wide">REMOVE</button>
    </div>
    )
  })
}
